...
# Script para corrigir erro do PPA deadsnakes no Ubuntu 24.04
# Sistema de Automação para Mineração de Bitcoin

set -e

echo "🔧 Corrigindo erro do PPA deadsnakes no Ubuntu 24.04"
...
if [ "$EUID" -ne 0 ]; then
    error "Execute como root: sudo ./fix_ppa_error.sh"
fi
...
if [[ "$UBUNTU_VERSION" == "24.04" ]]; then
    log "Ubuntu 24.04 detectado - corrigindo PPA..."
    ...
    # Instalar Python 3.12 (padrão do Ubuntu 24.04)
