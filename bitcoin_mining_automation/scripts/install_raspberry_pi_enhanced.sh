...
# Script de instalação aprimorado para Raspberry Pi
# Sistema de Automação para Mineração de Bitcoin
# Baseado no script original com melhorias e funcionalidades adicionais

set -e

echo "🚀 Instalando Sistema de Automação para Mineração de Bitcoin no Raspberry Pi (Versão Aprimorada)"
...
if [ "$EUID" -ne 0 ]; then
    error "Por favor, execute como root (sudo ./install_raspberry_pi_enhanced.sh)"
fi
...
if command -v hashcore &> /dev/null; then
    ...
    info "Para instalar HashCore Toolkit:"
...
cat > /opt/bitcoin_mining/README_RASPBERRY_PI_ENHANCED.md << 'EOF'
# Sistema de Automação para Mineração de Bitcoin - Raspberry Pi (Versão Aprimorada)

## Instalação Concluída

O sistema foi instalado com sucesso no seu Raspberry Pi com todas as funcionalidades do script original!
...
echo
echo "Documentação: /opt/bitcoin_mining/README_RASPBERRY_PI_ENHANCED.md"
echo
