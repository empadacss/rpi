...
if [ "$EUID" -ne 0 ]; then
    error "Execute como root: sudo ./quick_start_ubuntu.sh"
fi

# Verificar se o sistema já está instalado
if [ ! -d "/opt/bitcoin_mining" ]; then
    error "Sistema não instalado. Execute primeiro: sudo ./install_ubuntu_raspberry_pi.sh"
fi
...
echo
echo "Documentação: README_UBUNTU_RASPBERRY_PI.md"
echo "=========================================="
