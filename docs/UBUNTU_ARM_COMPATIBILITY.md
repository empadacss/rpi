...
O projeto principal está no diretório `bitcoin_mining_automation/` e fornece um backend em FastAPI, scripts de instalação e monitoramento, além de orquestração via Docker Compose para a automação de uma infraestrutura de mineração de Bitcoin. A arquitetura técnica detalha a integração entre backend, coleta de dados (ABB, ASICs, BLE), notificações e monitoramento com Prometheus/Grafana.【F:bitcoin_mining_automation/docs/TECHNICAL_ARCHITECTURE.md†L1-L88】 A partir da revisão atual, o script dedicado ao Ubuntu para Raspberry Pi copia (ou baixa automaticamente) o conteúdo do repositório para `/opt/bitcoin_mining`, garantindo que os arquivos reais de execução permaneçam centralizados nesse diretório após a instalação.【F:bitcoin_mining_automation/scripts/install_ubuntu_raspberry_pi.sh†L258-L315】 O `docker-compose.yml` segue a especificação atual do Compose (sem o campo `version`) para evitar avisos em ambientes modernos.【F:bitcoin_mining_automation/docker-compose.yml†L1-L178】
...
1. **Scripts de instalação** – O script dedicado ao Ubuntu em ARM verifica a arquitetura (`aarch64`/`armv7l`), instala dependências via `apt` (incluindo bibliotecas Modbus/BLE) e agora detecta automaticamente se o repositório da NodeSource suporta o codinome do Ubuntu, adotando o pacote nativo quando necessário. Também garante que `/opt/bitcoin_mining` receba uma cópia completa do repositório (com `docker-compose.yml`, `backend/` e `.env.example`), evitando erros de “arquivo inexistente” após a execução. Isso confirma suporte direto a Ubuntu ARM a partir da versão 20.04.【F:bitcoin_mining_automation/scripts/install_ubuntu_raspberry_pi.sh†L25-L214】【F:bitcoin_mining_automation/scripts/install_ubuntu_raspberry_pi.sh†L258-L317】
2. **Backend/Docker** – O backend usa base `python:3.11-slim`, disponível em ARM, e instala bibliotecas Python compatíveis. O `docker-compose.yml` define `LLM_MODE` como desabilitado por padrão, expõe o serviço Ollama atrás de um profile opcional e evita avisos removendo o atributo `version`. O guia de execução passou a incluir um resumo “copiar e colar” com os comandos essenciais, além de instruções para adicionar o usuário ao grupo Docker e acessar logs via `docker compose logs`.【F:bitcoin_mining_automation/docker-compose.yml†L1-L178】【F:bitcoin_mining_automation/GUIA_EXECUCAO_RASPBERRY_PI_UBUNTU.md†L21-L88】【F:bitcoin_mining_automation/GUIA_EXECUCAO_RASPBERRY_PI_UBUNTU.md†L167-L208】
